...
def verify_a11y():
    with sync_playwright() as p:
        ...
        assert back_link.get_attribute('aria-label') == 'Back to Home', f"Expected aria-label 'Back to Home', got '{back_link.get_attribute('aria-label')}'"
        back_icon = back_link.locator('span:has-text("arrow_back")')
        assert back_icon.get_attribute('aria-hidden') == 'true', "Expected aria-hidden 'true' on back icon"
